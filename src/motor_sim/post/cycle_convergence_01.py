...
def compute_cycle_metrics(df_cycle, y_end, ctx, S, cfg) -> dict:
    ...
    if 'cylinder_temperatures' in monitored:
        for cyl in ctx.cylinders:
            metrics[f'cylinder_temp__{cyl.name}_K'] = float(y_end[S.i(f'T__{cyl.name}')])
            metrics[f'intake_runner_temp__{cyl.name}_K'] = float(y_end[S.i(f'T_rin__{cyl.name}')])
            metrics[f'exhaust_runner_temp__{cyl.name}_K'] = float(y_end[S.i(f'T_rex__{cyl.name}')])
    ...
    if 'intake_plenum_temperature' in monitored:
        metrics['intake_plenum_temp_K'] = float(y_end[S.i('T_int_plenum')])
    if 'exhaust_plenum_temperature' in monitored:
        metrics['exhaust_plenum_temp_K'] = float(y_end[S.i('T_ex_plenum')])

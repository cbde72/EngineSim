...
def _build_cycle_records(t, Y, model, ctx, S, active, idx_m_active, idx_T_active, cyl_state_idx):
    records = []
    for j, ti in enumerate(t):
        ...
        rec["T_cyl_K"] = float(yj[idx_T_active])
        ...
        for cyl in ctx.cylinders:
            ...
            rec[f"{prefix}__T_rin_K_state"] = float(yj[idxs["T_rin"]])
            rec[f"{prefix}__m_rex_kg_state"] = float(yj[idxs["m_rex"]])
            rec[f"{prefix}__T_rex_K_state"] = float(yj[idxs["T_rex"]])
            rec[f"{prefix}__m_cyl_kg"] = float(yj[idxs["m_cyl"]])
            rec[f"{prefix}__T_cyl_K"] = float(yj[idxs["T_cyl"]])
...
def run_case(config_path: str) -> int:
    ...
    for cyl in ctx.cylinders:
        ...
        cyl_state_idx[prefix] = {
            ...
            "T_rin": S.i(f"T_rin__{prefix}"),
            "m_rex": S.i(f"m_rex__{prefix}"),
            "T_rex": S.i(f"T_rex__{prefix}"),
            "m_cyl": S.i(f"m__{prefix}"),
            "T_cyl": S.i(f"T__{prefix}"),
        }
    ...
    for cycle_idx in range(n_cycles_target):
        ...
        records_cycle = _build_cycle_records(
            t_seg, Y_seg, model, ctx, S, active, idx_m_active, idx_T_active, cyl_state_idx
        )

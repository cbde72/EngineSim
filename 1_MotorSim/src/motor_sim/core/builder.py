...
class ModelBuilder:
    ...
    def build(self):
        ...
        numerics_cfg = getattr(cfg, "numerics", {})
        state_representation = str(numerics_cfg.get("state_representation", "mT"))
        use_energy_states = state_representation.lower() == "mu"

        if use_energy_states:
            names = ["theta", "m_int_plenum", "U_int_plenum", "m_ex_plenum", "U_ex_plenum"]
            for cyl in cylinders:
                names += [f"m_rin__{cyl.name}", f"U_rin__{cyl.name}", f"m_rex__{cyl.name}", f"U_rex__{cyl.name}", f"m__{cyl.name}", f"U__{cyl.name}"]
        else:
        ...
        min_runner_volume_m3 = float(numerics_cfg.get("min_runner_volume_m3", 1e-9))
        for cyl in cylinders:

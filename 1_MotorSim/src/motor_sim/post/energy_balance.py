...
from .state_decode import temperature_series_from_columns, gas_cv
...
class WallHeatLumpedEnergyModel(EnergyTermModel):
    ...
    def compute_energy_J(self, df: pd.DataFrame, cfg_energy: dict, gas_R: float, gas_cp: float, angle_ref_mode: str = 'FIRE_TDC', crank_angle_offset_deg: float = 0.0) -> float:
        ...
        if len(df) == 0:
            return 0.0
        t = np.asarray(df["t_s"], dtype=float)
        T_cyl = temperature_series_from_columns(df, mass_col="m_cyl_kg", temp_col="T_cyl_K", energy_col="U_cyl_J_state", gas_R=gas_R, gas_cp=gas_cp, default_K=0.0) if len(df) else np.zeros(len(t))
...
def energy_balance_from_dataframe(df: pd.DataFrame, gas_R: float, gas_cp: float, cfg_energy: dict | None = None, angle_ref_mode: str = 'FIRE_TDC', crank_angle_offset_deg: float = 0.0) -> EnergySummary:
    ...
    T_in = temperature_series_from_columns(df, mass_col="m_int_plenum_kg", temp_col="T_int_plenum_K", energy_col="U_int_plenum_J", gas_R=gas_R, gas_cp=gas_cp, default_K=300.0) if ("T_int_plenum_K" in df.columns or "U_int_plenum_J" in df.columns) else (df["T_upstream_throttle_K"].to_numpy(dtype=float) if "T_upstream_throttle_K" in df.columns else 300.0 + 0.0 * t)
    T_cyl = temperature_series_from_columns(df, mass_col="m_cyl_kg", temp_col="T_cyl_K", energy_col="U_cyl_J_state", gas_R=gas_R, gas_cp=gas_cp, default_K=300.0)
    ...
    if "U_cyl_J_state" in df.columns:
        u = df["U_cyl_J_state"].to_numpy(dtype=float)
    else:
        u = cv * m * T_cyl

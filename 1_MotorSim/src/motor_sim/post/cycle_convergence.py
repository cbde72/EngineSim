...
from .state_decode import _detect_prefixes, scalar_temperature_from_rowlike
...
def compute_cycle_metrics(
    ...
    gas_R = float(getattr(ctx.gas, "R", 287.0))
    gas_cp = float(getattr(ctx.gas, "cp", 1005.0))
    metrics["temp"] = scalar_temperature_from_rowlike(
        df_cycle.iloc[-1],
        mass_key="m_cyl_kg",
        temp_key="T_cyl_K",
        energy_key="U_cyl_J_state",
        gas_R=gas_R,
        gas_cp=gas_cp,
        default_K=0.0,
    )
    ...
    plenum_temps = []
    if any(c in df_cycle.columns for c in ("T_int_plenum_K", "U_int_plenum_J")):
        plenum_temps.append(scalar_temperature_from_rowlike(df_cycle.iloc[-1], mass_key="m_int_plenum_kg", temp_key="T_int_plenum_K", energy_key="U_int_plenum_J", gas_R=gas_R, gas_cp=gas_cp, default_K=0.0))
    if any(c in df_cycle.columns for c in ("T_ex_plenum_K", "U_ex_plenum_J")):
        plenum_temps.append(scalar_temperature_from_rowlike(df_cycle.iloc[-1], mass_key="m_ex_plenum_kg", temp_key="T_ex_plenum_K", energy_key="U_ex_plenum_J", gas_R=gas_R, gas_cp=gas_cp, default_K=0.0))
    ...
    runner_temp_cols = [c for c in ("__T_rin_K_state", "__T_rex_K_state") if c in df_cycle.columns]
    if runner_temp_cols:
        metrics["runner_temp"] = _mean_columns(df_cycle, tuple(runner_temp_cols))
    else:
        runner_temps = []
        row = df_cycle.iloc[-1]
        for prefix in _detect_prefixes(df_cycle, ("__m_rin_kg_state", "__U_rin_J_state", "__m_rex_kg_state", "__U_rex_J_state")):
            if f"{prefix}__m_rin_kg_state" in df_cycle.columns and f"{prefix}__U_rin_J_state" in df_cycle.columns:
                runner_temps.append(scalar_temperature_from_rowlike(row, mass_key=f"{prefix}__m_rin_kg_state", temp_key=f"{prefix}__T_rin_K_state", energy_key=f"{prefix}__U_rin_J_state", gas_R=gas_R, gas_cp=gas_cp, default_K=0.0))
            if f"{prefix}__m_rex_kg_state" in df_cycle.columns and f"{prefix}__U_rex_J_state" in df_cycle.columns:
                runner_temps.append(scalar_temperature_from_rowlike(row, mass_key=f"{prefix}__m_rex_kg_state", temp_key=f"{prefix}__T_rex_K_state", energy_key=f"{prefix}__U_rex_J_state", gas_R=gas_R, gas_cp=gas_cp, default_K=0.0))
        metrics["runner_temp"] = float(np.mean(runner_temps)) if runner_temps else 0.0

...
class SimulationTab(BasicTab):
    def __init__(self):
        ...
        self.n_cycles_store = ConfigFieldFactory.ispin(2, 1, 100000, 1)
        self.n_cycles_compute = ConfigFieldFactory.ispin(5, 1, 100000, 1)
        ...
        fa.addRow("Stored cycles [-]", self.n_cycles_store)
        fa.addRow("Computed cycles [-]", self.n_cycles_compute)
    ...
    def set_config(self, cfg: dict) -> None:
        ...
        out = sim["output"]
        lp = sim["live_plot"]
        n_cycles_store = int(sim.get("n_cycles_store", sim.get("n_cycles", 1)))
        n_cycles_compute = int(sim.get("n_cycles_compute", n_cycles_store))
        self.t0.setValue(float(sim.get("t0_s", 0.0)))
        self.theta0.setValue(float(sim.get("theta0_deg", 0.0)))
        self.n_cycles_store.setValue(n_cycles_store)
        self.n_cycles_compute.setValue(max(n_cycles_compute, n_cycles_store))
    ...
    def update_config(self, cfg: dict) -> None:
        n_cycles_store = int(self.n_cycles_store.value())
        n_cycles_compute = max(int(self.n_cycles_compute.value()), n_cycles_store)
        cfg["simulation"] = {
            "t0_s": float(self.t0.value()),
            "theta0_deg": float(self.theta0.value()),
            "n_cycles_store": n_cycles_store,
            "n_cycles_compute": n_cycles_compute,
            "integrator": {
...
class ConfigEditorWindow(QMainWindow):
    ...
    def run_simulation(self) -> None:
        ...
        run_script = (Path(__file__).resolve().parents[3] / "scripts" / "run_simulation.py")
    ...
    def load_file_dialog(self) -> None:
        ...
        path, _ = QFileDialog.getOpenFileName(self, "Load configuration", start_dir, "JSON (*.json)")
        if not path:
            return
        try:
    ...
    def save_file(self, save_as: bool = False) -> None:
        ...
        if save_as or self.current_path is None:
            default_save = self.current_path if self.current_path else (Path.cwd() / "config_gui.json")
            path, _ = QFileDialog.getSaveFileName(self, "Save configuration", str(default_save), "JSON (*.json)")
            if not path:
                return
            self.current_path = Path(path)
        try:
